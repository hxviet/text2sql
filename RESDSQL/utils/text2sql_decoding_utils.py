...
from post_processing import fix_fatal_errors_in_sql
...
def decode_sqls(
    ...
    for batch_id in range(batch_size):
        ...
        for seq_id in range(num_return_sequences):
            ...
            # post process the query to fix fatal error
            old_sql = pred_sql
            pred_sql = fix_fatal_errors_in_sql(pred_sql, batch_tc_original[batch_id])
            if old_sql != pred_sql:
                print("Before fix: ", old_sql)
                print("After fix: ", pred_sql)
                print("---------------------")
