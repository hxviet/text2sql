...
def _train(opt):
    ...
    # save checkpoint for each 1 epochs
    num_checkpoint_steps = int(1 * len(train_dataset)/opt.batch_size)
    ...
    best_exec_plus_em = 0.00
    
    for epoch in range(opt.epochs):
        print(f"This is epoch {epoch+1}.")
        pbar = tqdm(train_dataloder, total=len(train_dataloder), desc=f'Training epoch {epoch + 1}')
        for batch in pbar:
            train_step += 1
        ...
        if epoch >= 5:
            ...
            if opt.target_type == "sql":
                ...
                cfg.device = "0"
                cfg.seed = 42
                cfg.save_path = "./models/text2sql-t5-base"
                cfg.eval_results_path = "./eval_results/text2sql-t5-base"
                cfg.mode = "eval"
                cfg.dev_filepath = "./data/preprocessed_data/resdsql_dev.json"
                cfg.original_dev_filepath = "./data/spider/dev.json"
                cfg.db_path = "./database"
                cfg.tables_for_natsql = "NatSQL/NatSQLv1_6/tables_for_natsql.json"
                cfg.num_beams = 8
                cfg.num_return_sequences = 8
                cfg.target_type = "sql"
                cfg.output = "predicted_sql.txt"
            elif opt.target_type == "natsql":
                cfg = edict()
                cfg.batch_size = 4
                cfg.device = "0"
                cfg.seed = 42
                cfg.save_path = "./models/text2natsql-t5-base"
                cfg.eval_results_path = "./eval_results/text2natsql-t5-base"
                cfg.mode = "eval"
                cfg.dev_filepath = "./data/preprocessed_data/resdsql_dev_natsql.json"
                cfg.original_dev_filepath = "./data/spider/dev.json"
                cfg.db_path = "./database"
                cfg.tables_for_natsql = "./data/preprocessed_data/tables_for_natsql.json"
                cfg.num_beams = 8
                cfg.num_return_sequences = 8
                cfg.target_type = "natsql"
                cfg.output = "predicted_sql.txt"
                
            # save latest checkpoint
            os.makedirs(cfg.save_path, exist_ok = True)
            os.makedirs(cfg.eval_results_path, exist_ok = True)
            
            model.save_pretrained(save_directory = opt.save_path + "/checkpoint-latest")
            text2sql_tokenizer.save_pretrained(save_directory = opt.save_path + "/checkpoint-latest")
            
            save_path = cfg.save_path

            # evaluate the latest checkpoint
            ckpt_name = "checkpoint-latest"
            print("Start evaluating ckpt: {}".format(ckpt_name))
            with open(cfg.eval_results_path+"/{}.txt".format(ckpt_name), "w") as f:
                f.write("Evaluating...")
            
            cfg.save_path = save_path + "/{}".format(ckpt_name)
            em, exec = _test(cfg)
            
            eval_result = dict()
            eval_result["ckpt"] = save_path + "/{}".format(ckpt_name)
            eval_result["EM"] = em
            ...
            if em > best_em or (em == best_em and exec > exec_in_best_em):
                ...
                model.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-em")
                text2sql_tokenizer.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-em")
                with open(cfg.eval_results_path+"/checkpoint-best-em.txt", "w") as f:
                    f.write(json.dumps(eval_result, indent = 2, ensure_ascii = False))
            ...
            if exec > best_exec or (exec == best_exec and em > em_in_best_exec):
                ...
                model.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-exec")
                text2sql_tokenizer.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-exec")
                with open(cfg.eval_results_path+"/checkpoint-best-exec.txt", "w") as f:
                    f.write(json.dumps(eval_result, indent = 2, ensure_ascii = False))
            
            if em + exec > best_exec_plus_em:
                best_exec_plus_em = em + exec
                
                model.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-em-plus-exec")
                text2sql_tokenizer.save_pretrained(save_directory = opt.save_path + "/checkpoint-best-em-plus-exec")
                with open(cfg.eval_results_path+"/checkpoint-best-em-plus-exec.txt", "w") as f:
                    f.write(json.dumps(eval_result, indent = 2, ensure_ascii = False))
            ######################################################################################
